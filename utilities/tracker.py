...
class Tracker(object):
    ...
    def __init__(self, dist_thresh=0.2,score_thresh=0.5,num_of_keypoints=5, max_humans=6):
        ''' 
        Arguments:
    ...
    def track(self, curr_skels,curr_boundingBoxes,curr_people):
        ...
        if len(self.id2skeleton) > 0:
            ...
            # returns dictionary of i2: i1 where i2 is skeleton id in current frame and i1 in previous frame
    ...
    def _match_features(self, features1, features2):
        ...
        def cost(sk1, sk2):
            ...
            joints = np.array([num for num in range(17)])

            sk1, sk2 = sk1[joints], sk2[joints]

            # take with score more than 0.05
            valid_idx = np.logical_and(sk1 >= 0.05, sk2 >= 0.05)
